...
const char* contentfulURL = "https://cdn.contentful.com/spaces/0tmn4pxpa056/environments/master/entries?content_type=sensores&select=fields.valor,fields.estado";
...
void scanRoutine() {
    ...
    if (httpResponseStatus > 0) {
        ...
        for(JsonObject item : items){
            int valor = item["fields"]["valor"] | -1;
            bool estado = item["fields"]["estado"] | false;

            if(valor >= 0) {
                pinMode(valor, OUTPUT);
                digitalWrite(valor, estado);
                Serial.printf("VALOR: %d, ESTADO: %s\n", valor, estado ? "ON" : "OFF");
            }
        }
    }
    else {
